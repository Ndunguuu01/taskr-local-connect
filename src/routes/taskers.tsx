          ...
          <CardContent className="grid gap-4 p-4 md:grid-cols-5">
            <div className="space-y-1">
              ...
              <Select value={category} onValueChange={setCategory}>
                ...
                <SelectContent>{CATEGORIES.map((c) => <SelectItem key={c || "all"} value={c || "__all__"}>{c || "All categories"}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
        ...
        <div className="mt-6">
          {loading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {rows.filter((r) => category !== "__all__" || true).map((r) => (
                <Link key={r.user_id} to="/tasker/$userId" params={{ userId: r.user_id }}>
                  <Card className="h-full transition-shadow hover:shadow-md">
                    <CardContent className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold">{r.full_name ?? "Tasker"}</p>
